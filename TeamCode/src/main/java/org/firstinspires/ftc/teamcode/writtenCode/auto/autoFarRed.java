...
public class autoFarRed extends OpMode {
    ...
    /* ---------------- START POSE ---------------- */
    private final Pose startPose = new Pose(90, 9.000, Math.toRadians(0));
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            // Collecting on Path1 (start -> 131.663, 35.181), then go shoot on Path2
            case 1:
                if (!follower.isBusy()) {
            ...
            // Shoot at Path2 end (131.663, 35.181 -> 89.893, 12.471), then collect on Path3
            case 2:
                if (!follower.isBusy()) {
                    intakeController.currentStatus = IntakeController.IntakeStatus.COLLECT;
                    transferController.currentStatus = TransferController.TransferStatus.COLLECT;
            ...
            // Collecting on Path3 (89.893, 12.471 -> 132.191, 9.632), then 1s delay on case 4
            case 3:
                if (!follower.isBusy()) {
                    timerDelay.reset();
                    setPathState(4);
            ...
            // Shoot at Path4 end (132.191, 9.632 -> 89.680, 12.413), then collect on Path5
            case 5:
                if (!follower.isBusy()) {
                    intakeController.currentStatus = IntakeController.IntakeStatus.COLLECT;
                    transferController.currentStatus = TransferController.TransferStatus.COLLECT;
            ...
            // Collecting on Path5 (89.680, 12.413 -> 133.156, 16.865) done, then 1s delay on case 7
            case 6:
                if (!follower.isBusy()) {
                    timerDelay.reset();
                    setPathState(7);
            ...
            // Shoot at Path6 end (133.156, 16.865 -> 89.728, 12.309), then collect on Path7
            case 8:
                if (!follower.isBusy()) {
                    intakeController.currentStatus = IntakeController.IntakeStatus.COLLECT;
                    transferController.currentStatus = TransferController.TransferStatus.COLLECT;
            ...
            // Collecting on Path7 (89.728, 12.309 -> 133.469, 19.915) done, then 1s delay on case 10
            case 9:
                if (!follower.isBusy()) {
                    timerDelay.reset();
                    setPathState(10);
            ...
            // Shoot at Path8 end (133.469, 19.915 -> 89.538, 12.410), then park on Path9
            case 11:
                if (!follower.isBusy()) {
                    intakeController.currentStatus = IntakeController.IntakeStatus.COLLECT;
                    transferController.currentStatus = TransferController.TransferStatus.COLLECT;
            ...
            // Park (Path9: 89.538, 12.410 -> 104.740, 12.312)
            case 12:
                if (!follower.isBusy()) {
                    setPathState(-1);
                }
        ...
        public Paths(Follower follower) {
            Path1 = follower.pathBuilder().addPath(
                            new BezierCurve(
                                    ...
                                    new Pose(85.642, 41.803),
                                    new Pose(131.663, 35.181)
                            )
